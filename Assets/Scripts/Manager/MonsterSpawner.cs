...
public class MonsterSpawner : MonoBehaviour
{
    ...
    private IEnumerator CoSpawn(DT_SpawnData val)
    {
        if (spawnCharacter != null)
        {
            for (int i = 0; i < val.spawnRepeat; i++) //SpawnCount
            {
                ...
                monsterStatus = spawnCharacter.gameObject.GetComponent<MonsterStatus>();
                monsterStatus.SetID(val.monster);

                monster = Instantiate(spawnCharacter, val.spawnPosition, UnityEngine.Quaternion.identity);
    ...
    {
        List<Dictionary<string, object>> Table = CSVReader.Read("Table_Spawn_2");
